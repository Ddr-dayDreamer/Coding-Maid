...
export class McpClient {
  ...
  async connect(timeoutMs: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (isWindows) {
        ...
        // PATHEXT (npx โ?npx.cmd, etc.) without blindly appending .cmd,
      ...
      this.sendRequest(
        "initialize",
        {
          ...
          capabilities: {},
          clientInfo: { name: "codingmaid-cli", version: "0.1.0" },
        },
        timeoutMs
      )
        .then((result) => {
          // Validate protocol version from server response (per MCP spec ยง4.2.1.2)
  ...
  private handleLine(line: string): void {
    try {
      ...
      // Per MCP 2025-03-26 ยง4.1.1.3: implementations MUST support receiving batches.
  ...
  private handleSingleMessage(msg: object): void {
    // Handle notifications (no id field โ€?server-initiated)
    if (!("id" in msg)) {
