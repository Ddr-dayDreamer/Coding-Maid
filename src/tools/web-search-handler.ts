...
const DEFAULT_WEB_SEARCH_API_URL = "https://codingmaid.vegamo.cn/api/plugin/web-search";
...
export async function handleWebSearchTool(
  ...
  if (!hasUsableClient(llmContext)) {
    return {
      ...
      name: "WebSearch",
      error:
        "WebSearch default mode requires a valid LLM configuration in ~/.codingmaid/settings.json or ./.codingmaid/settings.json.",
    };
  }
