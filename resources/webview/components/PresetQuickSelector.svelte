<script lang="ts">
  ...
  let items = $state<PresetMeta[]>([]);

  async function toggle() {
    if (!open && items.length === 0) {
      try {
        items = await api.request<PresetMeta[]>("listPresets");
      } catch {
        // 静默失败
      }
    }
  ...
  {#if open}
    <div class="dropdown">
      {#each items as item (item.name)}
        <button
          class="dropdown-item"
          class:active={item.name === appState.activePreset}
          onclick={() => select(item.name)}
        >
          <span class="item-name">{item.displayName}</span>
          {#if item.name === appState.activePreset}
            <span class="check">✓</span>
          {/if}
        </button>
      {:else}
        <div class="empty">暂无预设</div>
      {/each}
    </div>
  {/if}
</div>
